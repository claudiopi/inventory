...
@Component({
  ...
  standalone: true,
  imports: [FormsModule, FormsModule, IGX_CARD_DIRECTIVES, IGX_COMBO_DIRECTIVES, IGX_DIALOG_DIRECTIVES, IGX_DROP_DOWN_DIRECTIVES, IGX_INPUT_GROUP_DIRECTIVES, IGX_SELECT_DIRECTIVES, IGX_SIMPLE_COMBO_DIRECTIVES, IgxButtonDirective, IgxCheckboxComponent, IgxIconButtonDirective, IgxIconComponent, IgxOverlayOutletDirective, IgxRippleDirective, IgxToggleActionDirective, IgxToggleDirective],
...
export class ProductsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.inventoryAppService.getProductsList().pipe(takeUntil(this.destroy$)).subscribe(data => this.inventoryAppProducts = data);
